...
import com.bocs.alipay.trade.model.builder.*;
import com.bocs.alipay.trade.model.result.AlipayF2FPayResult;
import com.bocs.alipay.trade.model.result.AlipayF2FPrecreateResult;
import com.bocs.alipay.trade.model.result.AlipayF2FQueryResult;
import com.bocs.alipay.trade.model.result.AlipayF2FRefundResult;
import com.bocs.alipay.trade.service.AlipayTradeService;
import com.bocs.alipay.trade.utils.Utils;
...
abstract class AbsAlipayTradeService extends AbsAlipayService implements AlipayTradeService {
    ...
    protected AlipayTradeQueryResponse loopQueryResult(AlipayTradeQueryRequestBuilder builder) {
        ...
        for (int i = 0; i < PropertiesUtil.getInt("maxQueryRetry"); i++) {
            Utils.sleep(PropertiesUtil.getLong("queryDuration"));
    ...
    protected void asyncCancel(final AlipayTradeCancelRequestBuilder builder) {
        executorService.submit(new Runnable() {
            @Override
            public void run() {
                for (int i = 0; i < PropertiesUtil.getInt("maxCancelRetry"); i++) {
                    Utils.sleep(PropertiesUtil.getLong("cancelDuration"));
